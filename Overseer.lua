--[[
	Overseer UI — VoidUi-based Drawing library with mobile scaling.
	Executor: loadstring(game:HttpGet("RAW_URL/Overseer.luau", true))()
...
local drawing = {} do
 ...
 local pointerPos = services.InputService:GetMouseLocation()
 ...
 if pointerPos.X >= posX and pointerPos.Y >= posY and pointerPos.X <= sizeX and pointerPos.Y <= sizeY then
 return true
 end
 ...
 if ispointer(input) and not mouseoverhighersquare(obj) then
 ...
 if ispointer(input) and (not mouseoverhighersquare(obj) or obj.Transparency == 0) then
...
local Scale = {}
Scale.BASE_SIZE = Vector2.new(500, 550)
Scale.MOBILE_THRESHOLD = 560

function Scale.IsMobile()
 return services.InputService.TouchEnabled
end

function Scale.GetViewport()
...
function Scale.ComputeUIScale(viewport, isMobile)
 local margin = isMobile and 12 or 24
 local base = Scale.BASE_SIZE
 local fit = math.min((viewport.X - margin * 2) / base.X, (viewport.Y - margin * 2) / base.Y)
 local minScale = isMobile and 0.7 or 0.85
...
local library = utility.table({theme = table.clone(themes.Default), folder = "overseerui", extension = "overseer", flags = {}, open = true, keybind = Enum.KeyCode.RightShift, mousestate = services.InputService.MouseIconEnabled, cursor = nil, holder = nil, reopenbtn = nil, connections = {}}, true)
...
function library:Load(options)
 ...
 local baseSizeX = options.sizex or 500
 local baseSizeY = options.sizey or 550
...
local Overseer = {}
Overseer.Scale = Scale

function Overseer:Init()
 return library
end

function Overseer.Load(options)
 return library:Load(options)
end
